...
1. docs/index.md の「バックナンバー」セクションで、今日の日付リンクを該当月へ新しい順に追記する。最新月だけを `### YYYY年M月` 見出しで常時表示し、それ以前の月は `<details markdown="1">` と `<summary>YYYY年M月</summary>` で月単位に折りたたむ。月が変わったら、それまでの最新月も折りたたみへ変更する
2. mainブランチに対して git add → commit → push すること（claude/ で始まるブランチは使わないこと）
3. mainブランチにコミットしてプッシュされたことを確認すること、されていなければコミットしてプッシュすること
